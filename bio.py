...
if __name__ == "__main__":
    app.run()
# ================= Broadcast Command ===================
@app.on_message(filters.command("broadcast") & filters.user(6827299764))
async def broadcast(client, message):
    ...
    if args[1] in ["-pin", "-user"]:
        if len(args) < 3:
            return await message.reply_text(
                "Usage: /broadcast -pin <msg> or /broadcast -user <msg>"
            )
        flag = args[1]
    ...
    # Counters
    total_users = total_groups = sent_users = sent_groups = pinned_users = pinned_groups = failed_users = failed_groups = 0
    ...
    for uid, in rows:
        try:
            ...
            if chat.type in ("group", "supergroup"):
                ...
                if flag == "-pin":
                    try:
                        await client.send_message(uid, "📌 [PINNED]\n" + broadcast_text)
                    except: pass
                    pinned_users += 1
        except:
            if chat.type in ("group", "supergroup"):
...
@app.on_message(filters.command("fgcast") & filters.user(6827299764))
    ...
    for uid, in rows:
        try:
            ...
            if chat.type in ("group", "supergroup"):
                ...
                if flag == "-pin" and await bot_is_admin_with_permissions(client, uid, "can_pin_messages"):
                    try:
                        await fwd_msg.pin(disable_notification=True)
                    except: pass
                    pinned_groups += 1
            else:
                sent_users += 1
                if flag == "-pin":
                    try:
                        await client.send_message(uid, "📌 [PINNED] Forwarded message below:")
                    except: pass
                    pinned_users += 1
        except:
            if chat.type in ("group", "supergroup"):
